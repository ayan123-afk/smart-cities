...
/* ----- ENHANCED CAMERA CONTROLLER WITH MULTIPLE VIEWS ----- */
function EnhancedCameraController() {
  const { camera, gl } = useThree()
  const focus = useStore((s) => s.focus)
  const controlsRef = useRef()
  const [cameraMode, setCameraMode] = useState('orbit') // 'orbit', 'first-person', 'top-down', 'free'
  
  // Camera presets
  const cameraPresets = {
    'top-down': { position: [0, 50, 0], rotation: [-Math.PI / 2, 0, 0] },
    'first-person': { position: [0, 2, 10], rotation: [0, 0, 0] },
    'free': { position: [30, 20, 30], rotation: [0, 0, 0] }
  }
  ...
  }, [cameraMode, gl.domElement, camera])

  useFrame(() => {
    if (focus && cameraMode === 'orbit') {
  ...
  const switchCameraMode = (mode) => {
    ...
    if (cameraPresets[mode]) {
      camera.position.set(...cameraPresets[mode].position)
      camera.rotation.set(...cameraPresets[mode].rotation)
    }
  }
    ...
    <>
      ...
      <Html position={[0, 0, 0]}>
        <div style={{
          ...
          <h4 style={{ margin: '0 0 10px 0', color: '#3498db' }}>🎥 Camera Controls</h4>
          
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '10px' }}>
            <button 
              onClick={() => switchCameraMode('orbit')}
              style={{
                background: cameraMode === 'orbit' ? '#3498db' : '#2c3e50',
                color: 'white',
                border: 'none',
                padding: '8px',
                borderRadius: '6px',
            ...
            <button 
              onClick={() => switchCameraMode('first-person')}
              style={{
                background: cameraMode === 'first-person' ? '#e74c3c' : '#2c3e50',
                color: 'white',
                border: 'none',
                padding: '8px',
                borderRadius: '6px',
            ...
            <button 
              onClick={() => switchCameraMode('top-down')}
              style={{
                background: cameraMode === 'top-down' ? '#27ae60' : '#2c3e50',
                color: 'white',
                border: 'none',
                padding: '8px',
                borderRadius: '6px',
            ...
            <button 
              onClick={() => switchCameraMode('free')}
              style={{
                background: cameraMode === 'free' ? '#f39c12' : '#2c3e50',
                color: 'white',
                border: 'none',
                padding: '8px',
                borderRadius: '6px',
          ...
          {cameraMode === 'first-person' && (
            <div style={{ fontSize: '10px', color: '#ecf0f1', background: '#34495e', padding: '8px', borderRadius: '6px' }}>
              <div>🖱️ Click to lock mouse</div>
              <div>↕️ Move mouse to look around</div>
            </div>
          )}
    ...
    <div style={{ 
      ...
      <div style={{ 
        ...
        <div style={{ fontSize: 11, color: '#3498db', marginTop: 2 }}>
          👤 First Person: Click to lock mouse • Move mouse to look around
        </div>
        <div style={{ fontSize: 11, color: '#27ae60', marginTop: 2 }}>
          📊 Top Down: Bird's eye view of entire city
        </div>
        <div style={{ fontSize: 11, color: '#f39c12', marginTop: 2 }}>
          🕹️ Free Move: WASD to move • QE for height • Arrows to look
        </div>
      </div>
    </div>
  )
