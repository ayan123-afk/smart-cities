  ...
  return (
    <group position={position} rotation={rotation}>
      ...
      <mesh position={[0, 0.06, 0]} castShadow>
        ...
        <meshStandardMaterial color="#4488ff" metalness={0.8} roughness={0.1} />
      </mesh>
      <mesh position={[0, 0.12, 0]} castShadow>
        <boxGeometry args={[2.1, 0.05, 1.6]} />
        <meshStandardMaterial color="#1a1a2e" transparent opacity={0.3} metalness={0.9} />
      </mesh>
      <pointLight position={[0, 0.2, 0]} intensity={0.1} color="#4488ff" distance={2} />
    </group>
  );
}
...
function WindTurbine({ position = [0, 0, 0] }) {
  ...
  return (
    <group position={position}>
      ...
      <group ref={bladeRef}>
        {[0, 120, 240].map((angle) => (
          <mesh key={angle} position={[0, 10, 0]} rotation={[0, 0, angle * Math.PI / 180]}>
...
function WheelchairUser({ position = [0, 0, 0], moving = false }) {
  ...
  return (
    <group ref={groupRef} position={position}>
      ...
      <mesh position={[-0.3, 0.15, 0]} castShadow rotation={[0, 0, Math.PI / 2]}>
...
function Person({ position = [0, 0, 0], color = "#8b4513", speed = 0.2, path = null }) {
  ...
  useFrame((_, delta) => {
    if (groupRef.current && path && path.length > 1) {
      ...
      const newX = groupRef.current.position.x + (dx / distance) * step;
      const newZ = groupRef.current.position.z + (dz / distance) * step;

      if (Math.abs(newX - next[0]) < 0.1 && Math.abs(newZ - next[2]) < 0.1) {
        setPathIndex((prev) => (prev + 1) % path.length);
        groupRef.current.position.x = next[0];
        groupRef.current.position.z = next[2];
      } else {
...
function WasteBin({ position = [0, 0, 0], id = "bin1" }) {
  const timeOfDay = useStore((s) => s.timeOfDay);
  const wasteLevel = useStore((s) => s.wasteBins[id] || 20);
  const updateWasteBin = useStore((s) => s.updateWasteBin);
  const setAlertWasteManagement = useStore((s) => s.setAlertWasteManagement);
  const setCurrentBinTarget = useStore((s) => s.setCurrentBinTarget);
  const truckStatus = useStore((s) => s.truckStatus);
  const setTruckStatus = useStore((s) => s.setTruckStatus);
  const setCollectedWaste = useStore((s) => s.setCollectedWaste);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (wasteLevel < 100) {
        ...
        if (newLevel > 80 && !useStore.getState().alertWasteManagement) {
          setAlertWasteManagement(true);
        }
      }
    }, 5000);
    return () => clearInterval(interval);
  }, [wasteLevel, id, updateWasteBin, setAlertWasteManagement]);

  const handleClick = () => {
    setCurrentBinTarget(id);
    if (truckStatus === 'idle') {
      ...
      setTimeout(() => {
        ...
        setCollectedWaste(prev => prev + 100);
      }, 3000);
    }
  };
  ...
  return (
    <group position={position} onClick={handleClick}>
      ...
      <Text position={[0, 2, 0]} fontSize={0.3} color={color} anchorX="center">
      ...
      {timeOfDay === 'night' && (
        <pointLight position={[0, 2, 0]} intensity={0.3} color={color} distance={3} />
      )}
    </group>
  );
}
...
function WasteMonitoringSystem({ position = [0, 0, 0] }) {
  ...
  const truckStatus = useStore((s) => s.truckStatus);
  const collectedWaste = useStore((s) => s.collectedWaste);
  ...
  return (
    <group position={position}>
      ...
      {[0, 1, 2].map((i) => (
        <mesh key={i} position={[-1 + i, 0.8, 1.61]} castShadow>
      ...
      <Text position={[0, 2.3, 0]} fontSize={0.2} color="#3498db" anchorX="center">
        {alertWasteManagement ? '⚠️ High Waste!' : '✅ System Normal'}
      </Text>
      <Text position={[0, 2.8, 0]} fontSize={0.15} color="#ecf0f1" anchorX="center">
        Collected: {collectedWaste} kg
      </Text>
      {timeOfDay === 'night' && (
        <pointLight position={[0, 1, 1.5]} intensity={0.5} color="#3498db" distance={5} />
      )}
    </group>
  );
}
...
function MonitoringDrone({ position = [0, 0, 0], isMonitoring = false }) {
  ...
  const setMonitoringDrones = useStore((s) => s.setMonitoringDrones);
  const timeOfDay = useStore((s) => s.timeOfDay);
  ...
  return (
    <group ref={droneRef} position={position}>
      ...
      <mesh position={[0, 0.1, 0]} castShadow>
        <sphereGeometry args={[0.15, 8, 8]} />
        <meshStandardMaterial color={isMonitoring ? "#e74c3c" : "#2ecc71"} emissive={isMonitoring ? "#e74c3c" : "#2ecc71"} emissiveIntensity={0.5} />
      </mesh>
      {[0, 1, 2, 3].map((i) => {
        const angle = (i / 4) * Math.PI * 2;
        return (
          <mesh key={i} position={[Math.cos(angle) * 0.7, 0.1, Math.sin(angle) * 0.7]} castShadow>
      ...
      {timeOfDay === 'night' && (
        <pointLight position={[0, 0.2, 0]} intensity={0.5} color="#e74c3c" distance={2} />
      )}
    </group>
  );
}
  ...
  return (
    <group position={position}>
      ...
      {Array.from({ length: 6 }).map((_, i) => (
        <mesh key={i} position={[-4 + i * 1.6, -2.4, 0]} castShadow>
          <boxGeometry args={[1, 0.5, 4]} />
          <meshStandardMaterial color="#1a1a2e" metalness={0.6} />
        </mesh>
      ))}
      {Array.from({ length: 6 }).map((_, i) => (
        <mesh key={`light-${i}`} position={[-4 + i * 1.6, -2.7, 1.5]} castShadow>
      ...
      <Text position={[0, 3, 0]} fontSize={0.4} color="#00ff00" anchorX="center">
        🖥️ Data Center
      </Text>
      {timeOfDay === 'night' && (
...
function BusStation({ position = [0, 0, 0] }) {
  ...
  return (
    <group position={position}>
      ...
      {timeOfDay === 'night' && (
        <pointLight position={[0, 2, 0]} intensity={0.5} color="#f39c12" distance={5} />
      )}
    </group>
  );
}
...
function WasteManagementSystem({ position = [0, 0, 0] }) {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setWasteProcessing(prev => {
        const isProcessing = prev.isProcessing ? prev.processTime < 3 : Math.random() > 0.7;
        const newProcessTime = isProcessing ? prev.processTime + 0.5 : 0;
        const newRecycled = isProcessing ? prev.recycledWaste + 2 : prev.recycledWaste;
        const newReduced = isProcessing ? prev.reducedWaste + 1 : prev.reducedWaste;
        const newReused = isProcessing ? prev.reusedWaste + 1.5 : prev.reusedWaste;

        if (newProcessTime >= 3) {
          return { isProcessing: false, processTime: 0, recycledWaste: newRecycled, reducedWaste: newReduced, reusedWaste: newReused };
        }
        return { ...prev, isProcessing, processTime: newProcessTime, recycledWaste: newRecycled, reducedWaste: newReduced, reusedWaste: newReused };
      });
    }, 2000);
  ...
  return (
    <group position={position}>
      ...
      {[0, 1, 2].map((i) => (
        <mesh key={i} position={[-1.5 + i * 1.5, 0.8, 3.11]} castShadow>
          <planeGeometry args={[0.8, 0.3]} />
          <meshStandardMaterial
            color={wasteProcessing.isProcessing ? "#00ff00" : "#ff4444"}
            emissive={wasteProcessing.isProcessing ? "#00ff00" : "#ff4444"}
            emissiveIntensity={0.5}
          />
        </mesh>
      ))}
      <Text position={[0, 2.8, 0]} fontSize={0.35} color="white" anchorX="center">
        ♻️ Waste Management
      </Text>
      <Text position={[0, 3.4, 0]} fontSize={0.2} color="#ecf0f1" anchorX="center">
        Recycled: {Math.round(wasteProcessing.recycledWaste)} kg
      </Text>
      {timeOfDay === 'night' && (
        <pointLight position={[0, 1, 3]} intensity={0.5} color="#27ae60" distance={5} />
      )}
    </group>
  );
}
...
function EnergyEfficientSociety({ position = [0, 0, 0] }) {
  ...
  return (
    <group position={position}>
      ...
      {[0, 1, 2, 3].map((i) => (
        <mesh key={i} position={[-4 + i * 2.7, -2.8, 0]} castShadow>
          <boxGeometry args={[2, 4, 0.5]} />
          <meshStandardMaterial color="#2c3e50" />
        </mesh>
      ))}
      <SolarPanel position={[4, 3.5, 3]} rotation={[0, Math.PI / 4, 0]} />
      <SolarPanel position={[-4, 3.5, 3]} rotation={[0, -Math.PI / 4, 0]} />
      <SolarPanel position={[4, 3.5, -3]} rotation={[0, 3 * Math.PI / 4, 0]} />
      <SolarPanel position={[-4, 3.5, -3]} rotation={[0, -3 * Math.PI / 4, 0]} />
      <WindTurbine position={[5, 0, 0]} />
      <Text position={[0, 4.5, 0]} fontSize={0.4} color="#ecf0f1" anchorX="center">
        ⚡ Energy Society
      </Text>
      {timeOfDay === 'night' && (
        <pointLight position={[0, 3, 0]} intensity={0.5} color="#3498db" distance={5} />
      )}
    </group>
  );
}
    ...
    const interval = setInterval(() => {
      setSensorData(prev => {
        ...
        const newWaterLevel = isWatering ?
          ...
          Math.min(100, prev.waterLevel + 0.1);
